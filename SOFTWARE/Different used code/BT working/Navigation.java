...
public class Navigation {
	...
	UltrasonicSensor rightUSSensor = new UltrasonicSensor(SensorPort.S1);
	UltrasonicSensor leftUSSensor = new UltrasonicSensor(SensorPort.S4);
	...
	{
		turnTo(findAngle(30.48*xDest, 30.48*yDest));
	}
	
	public void travelToDestination()
	{
		travelTo(findAngle(30.48*xDest, 30.48*yDest));
	}
	
	public void travelTo(double x, double y)
	{
		travelTo(x,y,null,null);
	}
	
	public void travelTo(double x, double y, UltrasonicSensor leftUSSensor, UltrasonicSensor rightUSSensor) {
		double[] myPos = new double[3];
		boolean doNothing = false;
		if(this.leftUSSensor==null)
		{
	...
	private int getFilteredData() {
		...
		// do a ping
		forwardUSSensor.ping();
		
		// wait for the ping to complete
		try { Thread.sleep(50); } catch (InterruptedException e) {}
		
		// there will be a delay here
		distance = forwardUSSensor.getDistance();
		if (distance > 60)
			distance = 60;
	...
	private int getRightData() {
		...
		// there will be a delay here
		distance = rightUSSensor.getDistance();
		if (distance > 60)
			distance = 60;
	...
	private int getLeftData() {
		...
		// there will be a delay here
		distance = leftUSSensor.getDistance();
		if (distance > 60)
			distance = 60;
